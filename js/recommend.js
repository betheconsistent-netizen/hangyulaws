...
(function (global) {
  ...
  function getRecommendation(scorecard) {
    ...
    // 판단 보류 조건 3: Confidence Low → 해지 추천 금지
    ...
    if (score < CFG.SCORE_CANCEL_THRESHOLD) {
      return { label: LABELS.CANCEL,    canRecommendCancel: true,  score };
    }
    if (score < CFG.SCORE_DOWNGRADE_THRESHOLD) {
      return { label: LABELS.DOWNGRADE, canRecommendCancel: false, score };
    }
    return { label: LABELS.KEEP,      canRecommendCancel: false, score };
  }
